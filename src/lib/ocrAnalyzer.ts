...
// Parameter name patterns mapped to canonical names and units
const PARAM_PATTERNS: { regex: RegExp; name: string; unit: string }[] = [
  ...
  { regex: /\bkm\b|mean\s+k\b|km\s+\(/i,          name: 'Km',                 unit: 'D'   },
  ...
  { regex: /\bacd\b|ant.*chamber\s+dep/i,          name: 'ACD',                unit: 'mm'  },
  ...
  { regex: /\bppi-?avg\b/i,                        name: 'PPI-Avg',            unit: ''    },
  { regex: /\bppi-?min\b/i,                        name: 'PPI-Min',            unit: ''    },
...
// Resolve URL relative to the current page (works for both file:// and http://)
...
export async function analyzeWithOCR(
  ...
  const worker = await createWorker('eng', 1, {
    workerPath:  localUrl('tesseract/worker.min.js'),
    corePath:    localUrl('tesseract/tesseract-core.wasm.js'),
    langPath:    localUrl('tesseract/'),
  ...
  try {
    ...
    // Infer image dimensions from the bounding boxes of all words
    if (words.length > 0) {
  ...
  // Build lines: group words by approximate Y position (within 12px = same line)
  const lines: Word[][] = [];
  for (const word of words) {
    const cy = (word.bbox.y0 + word.bbox.y1) / 2;
    const existing = lines.find(
      (line) => Math.abs((line[0].bbox.y0 + line[0].bbox.y1) / 2 - cy) < 14
    );
    if (existing) {
      existing.push(word);
    } else {
  ...
  // For each line, join text and look for parameter name + value patterns
  const found: Map<string, { value: number; unit: string; x: number; y: number; w: number; h: number }> = new Map();

  for (const line of lines) {
    const lineText = line.map((w) => w.text).join(' ');

    for (const pat of PARAM_PATTERNS) {
      if (found.has(pat.name)) continue; // already found
      if (!pat.regex.test(lineText)) continue;

      // Find a number in this line (skip if it's just whitespace/punctuation)
      ...
      // Normalise bbox to 0-1 fractions
      found.set(pat.name, {
        value,
        unit: pat.unit,
        x: ((numWord.bbox.x0 + numWord.bbox.x1) / 2) / Math.max(imgWidth, 1),
        y: ((numWord.bbox.y0 + numWord.bbox.y1) / 2) / Math.max(imgHeight, 1),
        w: (numWord.bbox.x1 - numWord.bbox.x0) / Math.max(imgWidth, 1),
        h: (numWord.bbox.y1 - numWord.bbox.y0) / Math.max(imgHeight, 1),
      });
    }
  }

  // Also scan using a lookahead approach: name word followed by a number within 300px
  for (const pat of PARAM_PATTERNS) {
    ...
    for (let li = 0; li < lines.length; li++) {
      ...
      for (let wi = 0; wi < line.length; wi++) {
        const word = line[wi];
        if (!pat.regex.test(word.text)) continue;

        // Look ahead in this line and the next line for a number
        const candidates = [
          ...line.slice(wi + 1, wi + 5),
          ...(lines[li + 1] ?? []).slice(0, 4),
        ];
      ...
      if (found.has(pat.name)) break;
    }
  }

  if (found.size === 0) {
    throw new Error(
      'No corneal topography parameters could be extracted from this image. ' +
      'Make sure the image is a clear, unrotated screenshot from a topography device. ' +
      'For best results, screenshot only the data panel (not the colorful map).'
    );
  }

  const entries = Array.from(found.entries()).map(([name, info]) => ({
    name,
    value: info.value,
    unit: info.unit,
    x: info.x,
    y: info.y,
    width: Math.max(info.w, 0.04),
    height: Math.max(info.h, 0.02),
  }));

  // Detect device from OCR text
  ...
  return {
    ...buildResult(entries, device, eye),
    // Override parameters to include positions for canvas annotation
    parameters: buildResult(entries, device, eye).parameters.map((p, i) => ({
      ...p,
      x: entries[i]?.x ?? 0,
      y: entries[i]?.y ?? 0,
